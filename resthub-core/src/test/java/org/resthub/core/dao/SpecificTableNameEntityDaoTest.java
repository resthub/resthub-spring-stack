...
/**
 * This Test Suite performs tests on {@link SpecificTableNameEntity} class in
 * order to validate generic daos and services behaviours with entities named
 * differently than their classes
 */
public class SpecificTableNameEntityDaoTest extends
    ...
    public void testFindLike() throws Exception {
        ...
        assertFalse("entities should not empty", entities.isEmpty());
        assertEquals("entities size should be exactly 1", 1, entities.size());
        assertEquals("entitie name should be 'test'", ENTITY_NAME, entities.get(0).getName());
    }
