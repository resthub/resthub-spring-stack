...
public class InitializeDatabaseBeanDefinitionParser extends AbstractBeanDefinitionParser implements
    ...
    private BeanDefinition createSelectiveDatabasePopulator(Element element) {
        ...
        List<Element> exceptionElements = DomUtils.getChildElementsByTagName(element, "exception");
        ...
        BeanDefinitionBuilder selectivebuilder = BeanDefinitionBuilder
                .genericBeanDefinition(SelectiveDatabasePopulator.class);
        ...
        for (Element exception : exceptionElements) {
            ...
            List<Element> exceptionScripts = DomUtils.getChildElementsByTagName(exception, "script");
            selectivebuilder.addPropertyValue("exceptions[" + product + "]",
                    createResourceDatabasePopulator(exceptionScripts, ignoreFailedDrops, continueOnError));
        }

        return selectivebuilder.getBeanDefinition();
    }

    private BeanDefinition createResourceDatabasePopulator(List<Element> scripts, boolean ignoreFailedDrops,
