...
public class DbUnitTestExecutionListener implements TestExecutionListener {
    ...
    private Logger logger = LoggerFactory.getLogger(DbUnitTestExecutionListener.class);
    ...
    public void afterTestMethod(TestContext testContext) throws Exception {
        init(testContext);
        CleanupDB cleanupDB = testContext.getTestMethod().getAnnotation(CleanupDB.class);
        if (cleanupDB != null && (cleanupDB.position() == Position.AFTER || cleanupDB.position() == Position.BOTH)) {
    ...
    public void beforeTestMethod(TestContext testContext) throws Exception {
        init(testContext);
        CleanupDB cleanupDB = testContext.getTestMethod().getAnnotation(CleanupDB.class);
        if (cleanupDB != null && (cleanupDB.position() == Position.BEFORE || cleanupDB.position() == Position.BOTH)) {
            DbUnitConfiguration configuration = getRequiredDbUnitConfiguration(testContext);
            dbUnitUtils.cleanup(configuration, cleanupDB);
        }

        SkipInjection skipInjection = testContext.getTestMethod().getAnnotation(SkipInjection.class);
        if (skipInjection != null) {
            return;
        }
        InjectDataSet injectDataSet = testContext.getTestMethod().getAnnotation(InjectDataSet.class);
        if (injectDataSet == null) {
            // look for annotation on class
            InjectDataSet injectDataSetOnClass = testContext.getTestInstance().getClass()
                    .getAnnotation(InjectDataSet.class);
            if (injectDataSetOnClass != null && injectDataSetOnClass.onceForClass()
                    && injectedTestInstances.contains(testContext.getTestInstance().getClass())) {
                logger.debug("Test instance already initialized, skipping. " + testContext.getTestInstance());
            } else {
        ...
        if (injectDataSet != null) {
            dbUnitUtils.inject(getRequiredDbUnitConfiguration(testContext), injectDataSet);
        }
    }
    ...
    private DbUnitConfiguration getRequiredDbUnitConfiguration(TestContext testContext) {
        DbUnitContext contextAnnotation = testContext.getTestInstance().getClass().getAnnotation(DbUnitContext.class);

        DbUnitConfiguration configuration;

        if (contextAnnotation != null && StringUtils.hasText(contextAnnotation.value())) {
            configuration = testContext.getApplicationContext().getBean(contextAnnotation.value(),
    ...
    private void init(TestContext testContext) {
        if (dbUnitUtils == null) {
            Collection<DataSetResolver> resolvers = testContext.getApplicationContext()
                    .getBeansOfType(DataSetResolver.class).values();
            if (!resolvers.isEmpty()) {
                dbUnitUtils = new DBUnitUtils(new ArrayList<DataSetResolver>(resolvers));
            } else {
                Map<String, DatabaseInitializer> initializers = testContext.getApplicationContext().getBeansOfType(
                        DatabaseInitializer.class);
                InitializerDataSetResolver resolver = new InitializerDataSetResolver(
                        getRequiredDbUnitConfiguration(testContext), initializers);
