...
public class AclOauth2Test {
    ...
    public void shouldUnauthorizedAccessBeRejected() {
        ...
        // When accessing a protected resource
        response = server.path("api/secured").header(HttpHeaders.AUTHORIZATION, "OAuth " + tokenResponse.accessToken).post(ClientResponse.class, group);
        // Then the result is an error.
    ...
    public void shouldAccessProtectedWithPermissionBeProcessed() {
        ...
        group = server.path("api/secured").header(HttpHeaders.AUTHORIZATION, "OAuth " + tokenResponse.accessToken).post(Group.class, group);
        // When accessing an API with a permission.
        response = server.path("api/secured/" + group.getId()).header(HttpHeaders.AUTHORIZATION, "OAuth " + tokenResponse.accessToken).delete(ClientResponse.class);
        // Then the result is an error.
