 ...
 */
public class UserControllerWebTest extends
		AbstractControllerWebTest<User, Long> {
	...
	public void shouldAddRoleToUser() throws Exception {
		...
		u = resource().path("user").type(MediaType.APPLICATION_XML)
				.post(User.class, u);

		// When I associate the user and the role
		resource().path("user/name/" + u.getLogin() + "/roles/" + r.getName())
				.type(MediaType.APPLICATION_XML).put();

		// Then I get the user with this role
		String userWithRole = resource().path("user/login/" + u.getLogin())
				.accept(MediaType.APPLICATION_JSON).get(String.class);
		assertTrue("The user should contain the role",
				userWithRole.contains(r.getName()));
	}
	...
	public void shouldRemoveRoleFromUser() throws Exception {
		...
		String userWithRole = resource().path("user/login/" + u.getLogin())
				.accept(MediaType.APPLICATION_JSON).get(String.class);
		assertFalse("The user shouldn't contain the role",
				userWithRole.contains(r.getName()));
	}
	...
	public void shouldGetRolesFromUsers() throws Exception {
		...
		r1 = resource().path("role").type(MediaType.APPLICATION_XML)
				.post(Role.class, r1);
		r2 = resource().path("role").type(MediaType.APPLICATION_XML)
				.post(Role.class, r2);
		...
		u1 = resource().path("user").type(MediaType.APPLICATION_XML)
				.post(User.class, u1);
		u2 = resource().path("user").type(MediaType.APPLICATION_XML)
				.post(User.class, u2);
		u3 = resource().path("user").type(MediaType.APPLICATION_XML)
				.post(User.class, u3);
		u4 = resource().path("user").type(MediaType.APPLICATION_XML)
				.post(User.class, u4);

		// Given the association of the users with the roles
		// u1 with role1
		resource()
				.path("user/name/" + u1.getLogin() + "/roles/" + r1.getName())
				.type(MediaType.APPLICATION_XML).put();
		// u3 with role2
		resource()
				.path("user/name/" + u3.getLogin() + "/roles/" + r2.getName())
				.type(MediaType.APPLICATION_XML).put();
		// u4 with both role1 and role2
		resource()
				.path("user/name/" + u4.getLogin() + "/roles/" + r1.getName())
				.type(MediaType.APPLICATION_XML).put();
		resource()
				.path("user/name/" + u4.getLogin() + "/roles/" + r2.getName())
				.type(MediaType.APPLICATION_XML).put();

		// When I look for users with roles
		String user1Roles = resource()
				.path("user/name/" + u1.getLogin() + "/roles")
				.accept(MediaType.APPLICATION_JSON).get(String.class);
		String user2Roles = resource()
				.path("user/name/" + u2.getLogin() + "/roles")
				.accept(MediaType.APPLICATION_JSON).get(String.class);
		String user3Roles = resource()
				.path("user/name/" + u3.getLogin() + "/roles")
				.accept(MediaType.APPLICATION_JSON).get(String.class);
		String user4Roles = resource()
				.path("user/name/" + u4.getLogin() + "/roles")
				.accept(MediaType.APPLICATION_JSON).get(String.class);

		// Then the lists should only contain what I asked for
		assertTrue("The list of roles for user1 should contain role1",
				user1Roles.contains(r1.getName()));
		assertEquals("The list of roles for user2 should be empty", "[ ]",
				user2Roles);
		assertTrue("The list of roles for user3 should contain role2",
				user3Roles.contains(r2.getName()));
		assertTrue(
				"The list of roles for user4 should contain role1 and role2",
				user4Roles.contains(r1.getName())
	...
	public void cannotCreateTwiceTheSameUser() throws Exception {
		...
		ClientResponse response = resource().path("user")
				.type(MediaType.APPLICATION_XML).post(ClientResponse.class, u);
	...
	public void shouldManageToCheckUserIdentity() {
		...
		ClientResponse postAnswerCorrectPass = resource().path("user/checkuser")
				.queryParam("user", u.getLogin())
				.queryParam("password", password)
				.post(ClientResponse.class);
		ClientResponse postAnswerWrongPass = resource().path("user/checkuser")
				.queryParam("user", u.getLogin())
		...
		assertEquals("The identity check should fail", Status.NOT_FOUND,
				postAnswerWrongPass.getClientResponseStatus());
	}
}
