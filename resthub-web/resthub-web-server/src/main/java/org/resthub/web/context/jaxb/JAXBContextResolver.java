package org.resthub.web.context.jaxb;
...
import org.resthub.core.context.jaxb.JAXBElementListContextBean;
...
public class JAXBContextResolver implements ContextResolver<JAXBContext> {
     ...
     * Class logger
     */
    protected static Logger logger = LoggerFactory.getLogger(JAXBContextResolver.class);

    private JAXBElementListContextBean jaxbElementListContextBean;

    public void setJaxbElementListContextBean(JAXBElementListContextBean jaxbElementListContextBean) {
    ...
    public void setClasspathPatterns(String classpathPatterns) {
        logger.warn("ClasspathPattern definition found : jaxb-elements-scan base configuration is ignored !!");
    ...
    protected Set<String> findMatchingElementsFromPackage(String basePackage) {
        ...
        try {
            ...
            logger.warn("Error during scanning elements : cannot scan " + basePackage + ".", e);
        }
        if (logger.isInfoEnabled()) {
            logger.info(basePackage + " successfully scanned : " + resources.length + " elements found.");
        }
        return elements;
    }
    ...
    private void addElementsToJAXBContext(Set<String> elements) {
        try {
            ...
            for (String className : elements) {
                knownClasses[i++] = Thread.currentThread().getContextClassLoader().loadClass(className);
