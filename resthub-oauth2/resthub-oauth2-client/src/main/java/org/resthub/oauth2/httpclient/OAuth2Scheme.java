...
public class OAuth2Scheme extends RFC2617Scheme {
	...
	@Override
	// Refer to the following URL to understand why scope != realm http://www.ietf.org/mail-archive/web/oauth/current/msg01657.html
	...
	public Header authenticate(Credentials credentials, HttpRequest httpRequest) throws AuthenticationException {
    	...
    	if(!(credentials instanceof OAuth2Credentials)) {
    		throw new AuthenticationException("Credentials should be an instance of OAuth2Credentials but credential parameter provided is " + credentials.getClass().getSimpleName());
    	}
    	OAuth2Credentials oauth2Credentials = (OAuth2Credentials)credentials;
    	
    	if(oauth2Credentials.getTokenResponse() == null) {
	    	...
	    	// Copy params (proxy, certificate validation strategy ...) from sent request to authentication request
	        ...
	        parameters.add(new BasicNameValuePair(USERNAME_PARAMETER_NAME, oauth2Credentials.getUserPrincipal().getName()));
	        ...
	        try {
	            ...
	            OAuth2TokenResponse tokenResponse = (OAuth2TokenResponse)SerializationHelper.jsonDeserialize(responseEntity, OAuth2TokenResponse.class);
		...
		return new BasicHeader(HttpHeaders.AUTHORIZATION, this.getSchemeName() + " " + oauth2Credentials.getTokenResponse().accessToken);
	}
